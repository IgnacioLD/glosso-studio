...
class HomeViewModel(
    ...
    fun cancelSetup() {
        _uiState.update { it.copy(isInitialSetupRequired = false) }
    }

    fun cancelDownload() {
        _uiState.update { it.copy(isDownloadRequired = false, pendingLevelIndex = null) }
    }
        ...
        viewModelScope.launch {
            ...
            val levelStats = (0 until 6).map { levelIndex ->
                ...
                val total = if (downloader.isLevelDownloaded(levelIndex)) {
                    try {
                        repository.getSentenceCount(levelIndex, prefs.getTargetLanguage())
                    } catch (e: Exception) {
    ...
    fun setLanguage(code: String) {
        ...
        // No recognizer swap needed — a single multilingual model serves every language.
        checkInitialSetup()
    }
